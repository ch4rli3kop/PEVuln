...
class VariableRecoveryState(VariableRecoveryStateBase):
    ...
    def register_callbacks(self, concrete_states):
        ...
        for concrete_state in concrete_states:
            ...
            #concrete_state.inspect.add_breakpoint('engine_process', BP(enabled=True, action=self._ch_hook_engine_process))
    ...
    def _ch_hook_engine_process(self, state):
    ...
    def _hook_memory_write(self, state):
        ...
        stack_offset, uninitialized_value = self._ch_addr_to_stack_offset(mem_write_address)
        ...
        if mem_write_address.op == '__add__' and uninitialized_value is not None:
            variable_type = 'byte *' # TODO: identify int array
            variable_size = None # later calculated value
            variable_range = None
            variable_expr = mem_write_address.args
            self.variable_manager[self.func_addr].add_ch_pointer_variable({'expr':variable_expr, 'offset':stack_offset, 'index':uninitialized_value, 'size':variable_size, 'variable_type':variable_type, 'state_addr':state.addr})
        ...
        # Check variable offset if it appears lately
            ...
            variable = SimStackVariable(stack_offset, mem_write_length, base='bp',
                                        ident=self.variable_manager[self.func_addr].next_variable_ident('stack'),
                                        region=self.func_addr, ch_variable_type=variable_type, ch_variable_expr=variable_expr, ch_variable_range=variable_range
                                        )
    ...
    def _ch_addr_to_stack_offset(self, addr):
        """
        Convert an address to a stack offset.
        ...
        def _parse(addr):
            ...
            if addr.op == '__add__':
                ...
                annotated = [ True for annotated, _, _ in parsed if annotated is True ]
                if len(annotated) != 1:
                    # either nothing is annotated, or more than one element is annotated
                    raise ValueError()

                return True, sum([ offset for _, offset, _ in parsed ]), uninitialized_value
            elif addr.op == '__sub__':
                # __sub__ might have multiple arguments

                parsed = [ _parse(arg) for arg in args ]
                first_annotated, first_offset, _ = parsed[0]
                if first_annotated is False:
                    # the first argument is not annotated. we don't support it.
                    raise ValueError()
                if any([ annotated for annotated, _, _ in parsed[1:] ]):
                    # more than one argument is annotated. we don't support it.
                    raise ValueError()

                return True, first_offset - sum([ offset for _, offset, _ in parsed[1:] ]), uninitialized_value
            else:
                anno = next(iter(anno for anno in addr.annotations if isinstance(anno, StackLocationAnnotation)), None)
                if anno is None:
                    if addr.op == 'BVV':
                        return False, addr._model_concrete.value, uninitialized_value
                    # if addr.op == 'BVS':
                ...
                #     return True, 4, uninitialized_value
                return True, anno.offset, uninitialized_value

        # find the annotated AST
        try: annotated, offset, uninitialized_value = _parse(addr)
        except ValueError: return None, None

        if not annotated:
            return None, None, uninitialized_value

        return self._to_signed(offset), uninitialized_value

class VariableTracking(ForwardAnalysis, VariableRecoveryBase):

    def __init__(self, func, proj,max_iterations=20):
        """
    ...
    #
    def _ch_run_test(self):
        init_state = self.ch_proj.factory.call_state(addr=self.func.addr)
        init_state.inspect.add_breakpoint('mem_write', BP(enabled=True, action=self._ch_hook_memory_write))
        simgr = self.ch_proj.factory.simgr(init_state)
        simgr.run()

    def _ch_hook_memory_write(self, state):
        print(hex(state.addr))

    def _another_analyze(self):
        self._calculate_buffer_size()
        self._update_ch_pointer_variable_size()

    def _get_size_by_offset(self, offset):
        for svar in self.ch_stack_variables:
            if svar.offset == offset:
                return svar.size

    def _update_ch_pointer_variable_size(self):
        # print('stack_variables ',self.ch_stack_variables)
            
        #print(self.variables)
        #print(self.pointer_variables)
        # print('1 stack_variables ',self.ch_pointer_variables)

        # update ch_pointer_variables size
        for svar in self.ch_pointer_variables:
            new_size = self._get_size_by_offset(svar['offset'])
            svar['size'] = new_size
        # print('2 stack_variables ',self.ch_pointer_variables)
    ...
    def get_ch_pointer_variables(self):
        return self.ch_pointer_variables
    ...
    def _filter_ch_stack_variables(self, variables):
        for svar in variables:
            if isinstance(svar, angr.sim_variable.SimStackVariable):
                if -0x10000 < svar.offset < 0x10000:
